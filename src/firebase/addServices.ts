...
import { db } from "./firebase"; // Assuming firebase.ts or firebase.js is in the same directory

// Define servicesData outside the function and export it
export const servicesData = [
  ...
  },
  {
    ...
    image: "gs://plumb-perfect-5af21.firebasestorage.app/garbage-disposal.png",
...
// This function is still available if you use it elsewhere, for example, to populate Firestore
export async function addServicesToFirestore() {
  // Renamed for clarity if its primary purpose is Firestore
  try {
    const servicesCollectionRef = collection(db, "services");

    for (const service of servicesData) {
      // You might want to check if the service already exists before adding
      // or have a different script for one-time data seeding.
      const docRef = await addDoc(servicesCollectionRef, service);
      console.log("Service added/updated in Firestore with ID:", docRef.id);
    }

    console.log("All services processed for Firestore successfully!");
  } catch (error) {
    console.error("Error processing services for Firestore:", error);
  }
}
